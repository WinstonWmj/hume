...
# 这个sed命令的作用是：从变量$dataset（比如"libero_10_no_noops_1.0.0_lerobot"或"behavior"）中提取最前面的连续英文字母部分(即匹配^[a-zA-Z]+)，其他都去掉。
# 比如"libero_10_no_noops_1.0.0_lerobot"会得到"libero"，"behavior"会得到"behavior"。
